...
Game::Game(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPWSTR lpCmdLine, _In_ int nCmdShow)
{
	...
	light[0] = new SpotLight(vec3(1,0.2,0), vec3(0,-1,0));
	//light[1] = new DirLight(vec3(0,0,0), vec3(0, -1, 0));
	...
	//OBJECTS
	obj[0] = new GameObject(rm->get_IDK(), gfx, vec3(0.f, -2.f, 0.f), vec3(0.f, 0.f, 1.58), vec3(2.f, 2.0f, 2.0f));
	obj[1] = new GameObject(rm->get_IDK(), gfx, vec3(0.f, -5.f, 0.f), vec3(0.f, 0.f, 1.58f), vec3(2.f, 2.0f, 2.0f));
	...
	DCube = new DynamicCube(rm->get_Models("DCube.obj"), gfx, vec3(-10.f, 0.f, 10.f), vec3(1.f, 0.f, 0.f), vec3(2.f, 2.0f, 2.0f));
	/////LIGHT////////
...
void Game::Update()
{
	...
	if (getkey('V')) {
		...
		camera->setRotation(vec3(3.2, 0, 0));
		light[0]->getPos().x = 0;
		light[0]->getPos().y = 0;
		light[0]->getPos().z = 15;
		light[0]->getRotation().x = 3.2;
